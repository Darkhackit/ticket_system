...
const submitTicket = async () => {
    try {
        ...
        formData.append('ticket_files',form.value.ticket_files)
        formData.append('value',form.value.value)
        formData.append('email',form.value.email)
        formData.append('category',form.value.category?.id)
        formData.append('title',form.value.title)
        formData.append('help_topic',form.value.help_topic?.id)
        formData.append('priority',form.value.priority)
        formData.append('branch',form.value.branch?.id)
        await axios.post('/api/add-ticket',formData)
        processing.value = false
        notification.notify({
            type:'success',
        ...
        <div class="w-4/5 flex flex-col md:flex-row border bg-white rounded-md py-10">
            ...
            <div class="  bg-white  sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
                <form  class="space-y-6" @submit.prevent="submitTicket">
                    ...
                    <div class="space-y-3">
                        <label for="file" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Attach Document(s)</label>
                        <input type="file"  @change="getFiles"  id="email" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" placeholder="username@primeinsuranceghana.com" >
                        <p class="text-red-600 text-sm" v-if="error.ticket_files" >{{error.ticket_files[0]}}</p>
                    </div>
                    <div v-if="showMessage" class="space-y-2">
                        <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Title of your issue</label>
